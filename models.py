...
# Создает директорию для DB, если нет
...
class Date(BaseModel):
    # date_id = AutoField(primary_key=True)    # Если записи были удалены, можно не создавать ID
...
class Orders(BaseModel):
    ...
    service_type = CharField(max_length=100, null=True)
...
class Programs(BaseModel):
    ...
    price_detail_ids = CharField(max_length=50, null=True)
...
class News(BaseModel):
    news_id = AutoField(primary_key=True)
    title = CharField(max_length=255)
    content = CharField(max_length=3500, null=True)
    image_path = CharField(max_length=500, null=True)
    file_path = CharField(max_length=500, null=True)
    file_type = CharField(max_length=50, null=True)  # 'photo', 'document'

    is_published = IntegerField(default=0)
...
class NewsStats(BaseModel):
    newsstats_id = AutoField(primary_key=True)
    news = ForeignKeyField(News, backref='newsstats', on_delete='CASCADE')
    total_users = IntegerField(default=0)
    sent_successfully = IntegerField(default=0)
    failed_sent = IntegerField(default=0)
    ...
    try:
        with db:
            models = [
                ...
                Mentors, Retreats, Reviews, Programs, FAQ, News, NewsStats
            ]
...
def init_database():
    ...
    # Проверка существования файла DB
...
# При импорте модуля создаются только пустые таблицы
